...
ipmysql="define( 'DB_HOST', '192.168.33.10' );"
...
linea_host=$(grep -n "'DB_HOST'" /srv/www/wordpress/wp-config.php | cut -d: -f1 | head -n1)

grep -n "'DB_HOST'" /srv/www/wordpress/wp-config.php

echo "LA LINEA DE HOST ES: $linea_host"

sed -i '/'DB_HOST'/d' "$config_wordpress"  # Borra LA LINEA DB_HOST
sed -i "${linea_host}i $ipmysql" "$config_wordpress"  # Inserta el contenido del archivo con claves

cat $config_wordpress | grep -n "'DB_HOST'" /srv/www/wordpress/wp-config.php
